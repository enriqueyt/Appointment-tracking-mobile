    ...
    appointmentServices
        ...
        .then(function(data){ 
          ...
          if(data.error){
            ...
            $scope.attendedAppointments=data.data.filter(function(item){return item._id==true})[0].count;
            $scope.unattendedAppointments=data.data.filter(function(item){return item._id==false})[0].count;
          }
        }, function(err){
          var alert = $ionicPopup.alert({
